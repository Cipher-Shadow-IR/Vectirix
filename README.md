...
<img src="https://img.shields.io/badge/Built%20With-React%20%7C%20ReactFlow%20%7C%20FastAPI-black?style=for-the-badge" />
...
> Built with **React 18**, **ReactFlow**, **Zustand**, and a **FastAPI** backend for speed, modularity, and real-time graph analysis.
...
- 📊 **Pipeline Analysis** — Submit pipelines to a FastAPI backend for DAG validation and graph metrics
...
│   │   └── nodes/
│   │       ├── BaseNode.js          # Reusable node wrapper (header, handles, resizer)
│   │       ├── BaseNode.css         # Node & handle styling
│   │       ├── DraggableEdge.js     # Custom edge with adjustable midpoint
...
│   └── package-lock.json
├── backend/
│   └── main.py                     # FastAPI server (pipeline parsing & DAG validation)
├── README.md
└── .DS_Store
```
...
# In a separate terminal, start the backend (from backend/)
...
No environment variables are required. The frontend is configured to POST to `http://localhost:8000/pipelines/parse` by default.
...
4. **User submits** the pipeline via the "Run Pipeline Analysis" button, which POSTs the graph to the backend.
5. **Backend analyzes** the graph: counts nodes and edges, performs DFS cycle detection, and returns `{ num_nodes, num_edges, is_dag }`.
...
curl -X POST http://localhost:8000/pipelines/parse \
  ...
  -d '{"nodes":[{"id":"1","type":"customInput"},{"id":"2","type":"llm"}],"edges":[{"id":"e1-2","source":"1","target":"2"}]}'
```
